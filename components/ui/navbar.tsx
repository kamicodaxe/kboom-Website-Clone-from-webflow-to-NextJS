...
export const getClassNameForActiveRoute = (pathname: string, route: string) => {
...
const Navbar: React.FC<NavbarProps> & { Skeleton: React.FC } = ({ translations, pathname }) => {
	...
	return (
		<nav className="flex justify-between items-center py-5 px-8 static mx-auto">
			...
			<ul className="items-stretch hidden space-x-6 lg:flex text-xl font-bold text-gray-800 dark:text-white">
				<li className="flex flex-1">
					<Link
						...
						className={clsx(
							...
							getClassNameForActiveRoute(pathname, '/')
				...
				<li className="flex">
					<Link
						...
						className={clsx(
							"flex items-center px-4 -mb-1 hover:text-[#2ce5a7]",
							getClassNameForActiveRoute(pathname, '/blog')
				...
				<li className="flex">
					<Link
						...
						className={clsx(
							"flex items-center px-4 -mb-1 hover:text-[#2ce5a7]",
							getClassNameForActiveRoute(pathname, '/about')
						)}>{translations.navigation.about}</Link>
				</li>
			</ul>
			
			<div className="items-center flex-shrink-0  lg:flex">
				<Link href="/contact" className="self-center px-4 md:px-8 py-3 font-semibold text-gray-800 dark:text-white border-2 border-gray-800 dark:border-white">{translations.contact.header}</Link>
			</div>

			<MobileMenuTrigger translations={translations} pathname={pathname} />
		</nav>
	)
}
