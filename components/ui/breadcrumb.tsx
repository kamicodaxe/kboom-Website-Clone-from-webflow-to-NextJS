...
const Breadcrumb: React.FC<BreadcrumbProps> = ({
	...
	return (
		<nav aria-label="breadcrumb" className="p-4 dark:bg-gray-800 dark:text-gray-100">
			<ol className="flex h-8 space-x-2">
				...
				{
					routes.map((route) => (
						<li key={route.href} className="flex items-center space-x-2">
							...
							<Link href={route.href} className="flex items-center px-1 capitalize hover:underline">{route.title}</Link>
						</li>
					))
				}
